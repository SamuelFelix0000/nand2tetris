...
/**
 * Translates each field into its corresponding binary code.
 * Takes in result coming from the Parser.
 */
...
public class Code {
    ...
    /** Return the binary representation of what to compute in C command. */
    public String codeComp(String comp) {
        return compTable.get(comp);
    }

    /** Return the binary representation of where to jump to in C command. */
    public String jumpCode(String jump) {
        return jumpTable.get(jump);
    }

    /** Return the binary representation of where to store computed value in C command. */
